...
- Tested: service unit paths (malformed→fallback, fenced JSON→parsed, worse projection→improvement disabled, missing field→fallback), provider connection failure→503, e2e browser flow incl. fallback banner, factor panels, history persistence across reload, demo determinism, dark theme. KNOWN ISSUE RESOLVED (2026-07-04): initial key was 401; replaced with valid key — live path fully verified end-to-end (200 in ~6.4s, structured JSON parsed, results UI rendered from live Gemma 4 output, verdict history stored live result, demo path still deterministic). Results header now always shows an explicit source chip: "AI ANALYSIS — RESEARCH ENGINE NOT CONNECTED" (blue, live) or "DEMO ANALYSIS" (amber, demo/fallback). Note: testing_agent subagent is not available in this environment; verification was done via curl response inspection + scripted Playwright browser runs.

## Live Research Engine (2026-07-04)
- Architecture: Frontend → Analysis Service → Research Engine → Evidence Pack → Gemma 4 31B → Structured Analysis → existing Results UI.
- `backend/research_provider.py`: TavilySearchProvider (env TAVILY_API_KEY/TAVILY_BASE_URL, server-side only, replaceable; returns validated title/url/snippet/score or None).
- `backend/research_engine.py`: ResearchEngine — one bounded batch of 4 categorized searches (competitors/pricing/market/community), dedupe by URL, weak-score filter, direct source access (fetch top result per category; snippet-only → LOW, accessed official/pricing → MEDIUM/HIGH), produces EvidencePack {status: success|partial|unavailable, confidence, real counts, findings[{category, claim, source, sourceUrl, evidence, confidence}]}. No key → status "unavailable", zero fabrication.
- Gemma prompt now receives the evidence pack (untrusted-data delimiters, FACT/INFERENCE/UNKNOWN rules, no-invented-URLs rule); brutalReality/evidence items may carry sourceUrl — sanitized server-side against pack URLs (unknown URLs stripped). New optional solutionCoverage (Mostly unsolved/Partially solved/Well solved/Commoditized).
- Result source labels: "Live Research + AI Analysis" (green chip) / "Partial Research + AI Analysis" (amber) / "AI Analysis — Research Unavailable" (blue) / "Demo analysis" (amber).
- Results UI: research summary strip (real counts + confidence) + Research evidence cards with safe external links + "Analyst assessment" label separating AI evidence; Brutal Reality + Evidence cards show "Evidence-backed/Source ↗" links when URL present; evidence note adapts to research state.
- Tested (`backend/test_research_pipeline.py` retained): stubbed-provider pack (counts, URLs, confidence upgrades), pack-in-prompt assertion, live Gemma grounded in pack with all sourceUrls pack-sanitized, no-key endpoint → "AI Analysis — Research Unavailable" with zero fabricated evidence, browser e2e (chip, note, save flow, history, demo determinism 42/100, dark theme, no console errors).
- PENDING: ~~user to add a Tavily key~~ RESOLVED (2026-07-04): TAVILY_API_KEY added server-side; live research verified end-to-end for both mandated test ideas (todo app for developers → KILL IT 25/100, 16 findings; React context packages → KILL IT 22/100, 15 findings), real sources (lifestack.ai, efficient.app, teamwork.com, atlassian.com, packmind.com, augmentcode.com), pack-sanitized AI citations, green "LIVE RESEARCH + AI ANALYSIS" chip in UI, evidence-backed brutal reality links, why-this-score source references, save flow 22→45, history persistence. Fallback simulated (key removed → "AI ANALYSIS — Research Unavailable", zero fabricated evidence, AI analysis continues) and key restored. Note: testing_agent subagent is not available in this environment; verification was done via curl response inspection + scripted Playwright browser runs.
